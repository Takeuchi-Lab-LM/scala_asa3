 ...
 */
class VerbArgDict(categorys: category.Dict) {
  ...
  private def createVerbDict(lines: Iterator[String]): Dict = {
    ...
    for (line <- lines) {
      ...
      val verb = vth(2) //述語
      val sem = vth.slice(35, 40).mkString("-") //概念フレーム
      vth(41).contains("×") match { //コメントに"☓"があれば除外
        case false =>
  ...
  private def createSemantic(vths: List[String]): Semantic = {
    ...
    semantic.semantic = vths.slice(35, 40).mkString("-")
  ...
  private def createInstance(vths: List[String]): Instance = {
    ...
    for (n <- List(4, 10, 16, 22, 28)) {
      val role = vths(n) //深層格
      val arg = vths(n+1) //arg
      val parts = vths(n + 2).split("・").toList.filterNot(_.contains("?")) //表層格
      val noun = vths(n + 3).replace(parts(0), "") //事例
      ...
      parts.foreach { part =>
        cates.foreach { cate =>
          val cas = this.createCase(role, arg, part, noun, cate)
   ...
   */
  private def createCase(role: String, arg:String, part: String, noun: String, category: String): Case = {
    val cas = new Case
    cas.semrole = role
    cas.arg = arg
    cas.part = part
