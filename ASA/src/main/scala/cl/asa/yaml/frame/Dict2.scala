...
class Dict2(dic: String, yaml: String, snakeyaml: Yaml) {
	...
	//val is:java.io.InputStream = getClass().getClassLoader().getResourceAsStream(dic)
	...
	def getFrame(verb: String): Option[Frame] = {
		val frame = index.get(verb) match {
			case Some(fp) =>
				val str = this.loadString(fp._1, fp._2)
				val data = snakeyaml.loadAs(str, classOf[Frame])
				Some(data)
			case None => None
		}
		return frame
	...
	def getSemanticFirstInstance(verb: String, sem: String): Option[Instance] = {
		val semantic = index.get(verb) match {
			case Some(fp) =>
				val str = this.loadString(fp._1, fp._2)
				val data = snakeyaml.loadAs(str, classOf[Frame])
				data.frame.find(_.semantic.equals(sem)) match {
					case Some(frame) =>frame.instance.headOption
					case None =>None
				}
			case None => None
		}
	...
	def isFrame(verb: String): Boolean = {
		val bool: Boolean = index.contains(verb)
		return bool
	}
